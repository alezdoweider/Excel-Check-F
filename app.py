...
def main():
    ...
    # ---------- Campos adicionales AÑO, MES, DÍA, CUSTODIO ----------
    ...
    if st.button("Guardar Información"):
        ...
        st.markdown("### Información ingresada:")
        st.write(f"**AÑO:** {anio}")
        st.write(f"**MES:** {mes}")
        st.write(f"**DÍA:** {dia}")
        st.write(f"**CUSTODIO:** {custodio}")
    ...
    if uploaded_file:
        try:
            ...
            if caso_seleccionado:
                ...
                # Mostrar filas con selectbox de envase
